...
from .runtime import CodexRuntime, doctor
...
def parser():
    ...
    check = sub.add_parser("doctor", help="Check Codex CLI compatibility and login")
    ...
    audit.add_argument("--model", help="Optional Codex model; otherwise use the CLI default")
...
def main(argv=None):
    ...
    try:
        if args.command == "doctor":
            ...
            print(f"Ready: {binary}\nAuthentication is available. Audit runs use the configured Codex account.")
            return 0
        ...
        if args.command == "audit":
            ...
            run_dir = args.run
        result = run_agent(run_dir, CodexRuntime(binary), args.max_rounds, args.timeout, args.budget)
